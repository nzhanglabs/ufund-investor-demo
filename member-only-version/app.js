...
function syncSessionRole(email, fallbackRole) {
  const users = getUsers();
  const user = users.find((u) => (u.email || "").toLowerCase() === (email || "").toLowerCase());
  const role = user?.role && roles[user.role] ? user.role : fallbackRole;
  if (email && role && roles[role]) {
    setSession({ email, role });
  }
  return role && roles[role] ? role : "member";
}
...
const pages = {
  "start-here": {
    ...
    sections: [
      ...
      { h: "Club Member", p: "Includes all Qualified Access(Limited) documents, plus Active Deals full package rooms, Upcoming Opportunities, Past Deals, and Member Reports." }
    ]
  },
  "resource-center": {
  ...
  faq: {
    ...
    sections: [
      ...
      { h: "What unlocks Club Member access?", p: "Club enrollment and verification completion unlock member-only areas such as Active Deals full packages, Upcoming Opportunities, Past Deals, and Member Reports." },
...
const navSections = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: "Upcoming Opportunities", route: "pipeline", memberOnly: true },
...
function setRole(role) {
  if (ui.roleSelect) ui.roleSelect.value = role;
}

function resolveInitialRole(session) {
  if (!session?.email) return "member";
  const role = syncSessionRole(session.email, session.role);
  setRole(role);
  return role;
}
...
function renderHeader(route, role) {
  ...
  if (route === "member-vault" && role === "member") {
    ...
    ui.roleHint.textContent = "Club Member includes all Qualified Access(Limited) documents and adds Active Deals + Upcoming Opportunities.";
    return;
  }
  ...
  if (role === "member") {
    ...
    ui.roleHint.textContent = "Qualified Access(Limited): Resource Center + Qualified Library are open. Active Deals and Upcoming Opportunities remain locked.";
  }
}
...
function renderPipelineWatchlist() {
  ui.content.innerHTML = `
    ...
    <div class="pipeline-disclaimer">
      <strong>Important Notice (Upcoming Opportunities)</strong>
      <p>The Upcoming Opportunities is provided for informational purposes only and reflects opportunities currently under review.</p>
...
function renderContent(route, role) {
  ...
  if (route === "start-here") {
    if (role === "qualified") {
      ui.content.innerHTML = `
        ...
        <div class="list">
          ...
          <div class="item"><h4>What Club Member Unlocks</h4><p>Active Deals full package rooms, Upcoming Opportunities, Past Deals Library, and Member Reports.</p></div>
    ...
    ui.content.innerHTML = `
      ...
      <div class="list">
        ...
        <div class="item"><h4>Upcoming Opportunities</h4><p>High-level themes currently under review.</p><div style="margin-top:8px;"><a class="btn" href="${routeToFile("pipeline", role)}">Enter</a></div></div>
  ...
  if (route === "member-vault" && role === "qualified") {
    ui.content.innerHTML = `
      ...
      <div class="qa-grid">
        ...
        <article class="qa-card"><h4>Upcoming Opportunities</h4><p>Theme-level pipeline and selection standards.</p><div class="qa-actions"><a class="btn locked" href="javascript:void(0)">Enter (Locked)</a></div></article>
  ...
  if (route === "faq") {
    ui.content.innerHTML = `
      ...
      <div class="list">
        ...
        <div class="item"><h4>What extra access does Club Member provide?</h4><p>Club Member includes Active Deals full package rooms, Upcoming Opportunities, Past Deals library, and Member Reports.</p></div>
      </div>
  ...
  if (route === "member-vault" && role === "member") {
    ...
    ui.content.innerHTML = `
      ...
      <div class="qa-grid">
        ...
        <article class="qa-card"><h4>Upcoming Opportunities</h4><p>Theme-level pipeline and selection standards.</p><div class="qa-actions"><a class="btn" href="${routeToFile("pipeline", role)}">Enter</a></div></article>
    ...
    if (!appState.hasActiveDeal) {
      ui.content.innerHTML = `
        ...
        <div class="lock-panel">
          ...
          <p style="margin:0;">There are currently no live deals open for review. Please check Upcoming Opportunities and Member Reports for updates.</p>
        </div>
  ...
  if (route === "logout") {
    ...
    window.location.href = routeToFile("member-vault", "member");
    return;
  }
  ...
  if (!canAccess(route, role)) {
    ui.content.innerHTML = `
      ...
      <div class="disclosure">Demo rule: qualified but unpaid users cannot open Active Deals and Upcoming Opportunities.</div>
    `;
    return;
  }
...
async function bootstrap() {
  ...
  const session = getSession() || { email: "demo.member@ufundinvestment.com", role: "member" };
  if (!getSession()) setSession(session);

  const role = resolveInitialRole(session);

  if (isIndexPage) {
    const homeRoute = "member-vault";
    window.location.href = routeToFile(homeRoute, role);
    return;
  }

  if (!pages[route]) {
    const fallbackRoute = "member-vault";
